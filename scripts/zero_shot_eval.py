...
def main():
    ...
    model = PET(num_feat=val_loader.num_feat,
                num_jet=val_loader.num_jet,
                num_classes=val_loader.num_classes,
                local=flags.local,
    ...
    optimizer_head = configure_optimizers(flags, val_loader)
    optimizer_body = configure_optimizers(flags, val_loader,lr_factor=flags.lr_factor if flags.fine_tune else 1.0)
    model.compile(optimizer_body,optimizer_head)

    # Evaluate on validation set
    val_data = val_loader.make_tfdata()
    val_steps = val_loader.steps_per_epoch
